...
var Editor = React.createClass({
  ...
  handleKeyUp(e) {
    ...
    // execute commands that match key down
    var { content, selection, figureId } = EditorStore.get();
    this.keys.execute(e, content, selection, this.updateResults);
  ...
  checkFigureSelection(e) {
    var target = e.target;
    var figureId = target.getAttribute('data-figureid')
    EditorStore.set({ figureId }, false);
  },
  ...
  render() {
    var { content, selection, linkState, figureId } = EditorStore.get();
    ...
    return (
      ...
      >
        <Content content={content} figureId={figureId} />
