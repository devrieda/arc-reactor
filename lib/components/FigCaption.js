var React = require('react/addons');
var PureRenderMixin = React.addons.PureRenderMixin;
var cx = require('classnames');

var KEY_CODES = { 'return': 13, 'bspace': 8 };

var { string, bool } = React.PropTypes;

var FigCaption = React.createClass({
  mixins: [PureRenderMixin],

  propTypes: {
    text: string,
    figSelected: bool
  },

  getDefaultProps() {
    return {
      text: '',
      figSelected: false
    };
  },

  getInitialState() {
    return {
      empty: this.props.text.length === 0
    };
  },

  // handle clicking out without setting a caption
  componentDidMount() {
    document.addEventListener("click", this.handleDocumentClick);
  },
  componentWillUnmount() {
    document.removeEventListener("click", this.handleDocumentClick);
  },
  handleDocumentClick(e) {
    var caption = React.findDOMNode(this.refs.caption);

    if (!e.target.classList.contains('ic-Editor-FigCaption__placeholder') &&
        e.target !== caption && !caption.contains(e.target)) {
      this.setState({empty: this.props.text.trim().length === 0});
    }
  },


  // clear out placeholder text
  handleClickPlaceholder() {
    if (this.state.empty) {
      this.setState({empty: false});
    }
  },

  // don't save placeholder text
  handleKeyUp(e) {
    var caption = React.findDOMNode(this.refs.caption);
    var placeholder = caption.getAttribute('data-placeholder');
    if (placeholder === caption.textContent.trim()) {
      e.stopPropagation();
    }
  },

  // don't allow us to move outside of caption text
  handleKeyDown(e) {
    // don't prevent any keyDown events when the parent figure is selected
    if (this.props.figSelected) { return; }

    // hide placeholder when they start typing anything
    this.handleClickPlaceholder();

    // only allow deletion of actual caption text
    var sel = document.getSelection();
    var beg = sel.anchorOffset === 0 && sel.type === 'Caret';

    var noBspace = beg && e.keyCode === KEY_CODES.bspace;
    if (noBspace || e.keyCode === KEY_CODES.return) {
      e.preventDefault();
      e.stopPropagation();
    }
  },

  classNames() {
    return cx({
      'ic-Editor-FigCaption': true,
      'ic-Editor-FigCaption--empty': this.props.text.trim().length === 0
    });
  },

  render() {
    var placeholder = "Type caption for image (optional)";

    return (
      <figcaption
        ref="caption"
        contentEditable="true"
        className={this.classNames()}
        onKeyDown={this.handleKeyDown}
        onKeyUp={this.handleKeyUp}
        onClick={this.handleClickPlaceholder}
        data-placeholder={placeholder}
        data-caption="true"
        tabIndex="-1"
      >
        {!this.state.empty &&
          (this.props.text.length > 0 ? this.props.text : "\u00a0")
        }

        {this.state.empty &&
          <span className="ic-Editor-FigCaption__placeholder">
            {placeholder}
          </span>
        }
      </figcaption>
    );
  }
});

module.exports = FigCaption;
