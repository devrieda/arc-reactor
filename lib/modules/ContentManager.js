...
class ContentManager {
  ...
  toggleCenter(guids, offsets) {
    var range = this.finder.findRange(guids, offsets)
  ...
  toggleMarkup(guids, offsets, type, value) {
    ...
    // no markups yet for this block
    if (block.markups[type].length === 0) {
      return block.markups[type].push(markup);
    }

    // this can't get too huge or we'll exceed call stack
    var slots = Array.apply(null, Array(block.text.length)).map(Boolean);
    console.log(slots.length)

    // ranges overlap
    if (this._markupRangesOverlap(block.markups[type], markup)) {


    // no overlaps - add to ranges
    } else {
      return block.markups[type].push(markup);
    }
  }

  _markupRangesOverlap(existing, selection) {
    for (var i = 0, j = existing.length; i < j; i++) {
      var range = existing[i];
      if (range.range[0] <= selection.range[0] ||
          range.range[1] >= selection.range[1]) {
        return true;
      }
    }
    return false;
