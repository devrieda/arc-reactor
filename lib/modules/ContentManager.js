...
class ContentManager {
  ...
  splitBlock(guids, offsets) {
    ...
    if (guids.anchor == guids.focus) {
      var block = this.finder.findBlock(guids.anchor);
      var text  = block.text;

      var beg = text.substring(0, offsets.anchor);
      var end = text.substring(offsets.focus);

      block.text = beg;
      var newBlock = this._insertBlock('p', 'after', guids.anchor, end);

    // TODO - handle cross-block selections
    } else {
      var anchor = this.finder.findBlock(guids.anchor);
      var focus  = this.finder.findBlock(guids.focus);
