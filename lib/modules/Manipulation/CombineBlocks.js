...
class CombineBlocks {
  ...
  execute(guids, offsets) {
    ...
    var focusPath  = last ? this._finder().findPath(last) : anchor;
    ...
    return { content: this.map.toJS(), block: anchor, offset: offsets.anchor };
  }
  ...
  _finder() {
    return new ContentFinder(this.content, this.map);
  }
}
