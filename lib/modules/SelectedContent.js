...
class SelectedContent {
  ...
  isActive(button) {
    if (button.type == 'center') {
      return this.isCentered()
    } else {
      return this.hasType(button.type);
    }
  }
  ...
  _hasInlineType(type) {
    ...
    if (blocks.length == 1) {
      var matching = ranges[0].filter( (range) => {
        return range.range[0] <= offsets.anchor &&
               range.range[1] >= offsets.focus;
      });
  ...
  _markupForAll(blocks, type) {
    for (var i = 0, j = blocks.length; i < j; i++) {
      ...
      if (range.range[0] > 0 || range.range[1] < block.text.length) {
        return false;
      }
    }
    return true;
  ...
  _hasMarkup(block, type, from, to) {
    ...
    for (var i = 0, j = markups.length; i < j; i++) {
      var range = markups[i];
      if (from >= range.range[0] && to <= range.range[1]) { return true; }
    }
    return false;
  }
