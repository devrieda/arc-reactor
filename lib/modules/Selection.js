...
class Selection {
  ...
  // get some meta info about the selection to help with events
  _initFigure() {
    if (!this.target) return;
    this.figureId = this.target.getAttribute('data-figureid');

    // remove focus from caption while image is selected
    if (this.figureId) {
      var node = this.target.firstChild;
      this._setRange(node, node, 0, 0);
    }
  }
  clearFigureId() {
    this.figureId = null;
    this.focusOn(this.anchor.guid, 0);
    this.reselect();
  }
