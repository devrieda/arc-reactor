...
class SpecialKey {
  ...
  execute() {
    specialStub();
    return {
      content: {a: 1}, selection: "b", stopPropogation: true
    };
  }
}
...
class MagicKey {
  ...
  execute() {
    magicStub();
    return {
      content: {b: 2}, selection: "d", stopPropogation: true
    };
  }
}
...
describe('Commands', () => {
  describe('.constructor', () => {
    it('should install default commands', () => {
      ...
      expect(commands.getCommands().length).toEqual(10);
    });
  });
  ...
  describe('#use', () => {
    it('should add command to stack', () => {
      ...
      expect(list[10]).toBe(SpecialKey);
    });
